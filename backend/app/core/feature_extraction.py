...
def extract_time_domain_features(signal_data):
    ...
    # Zero Crossing Rate (normalized by signal length so it's scale-invariant)
    n = len(signal_data)
    zero_crossings = int(np.sum(np.diff(np.signbit(signal_data))))
    zero_crossing_rate = zero_crossings / n if n > 0 else 0.0
...
def extract_frequency_domain_features(freq, magnitude, fault_freqs):
    ...
    if total_energy > 0:
        low_freq_energy_ratio = float(np.sum(magnitude[low_freq_idx] ** 2) / total_energy)
    else:
